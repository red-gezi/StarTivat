...
public class SelectManager : MonoBehaviour
{
    ...
    static SkillData currentActionData;
    ...
    public static void Show(SkillData actionData)
    {
    ...
    {
       BattleManager.CurrentBattle.charaList.ForEach(chara => chara.largeLock.SetActive(false));
       BattleManager.CurrentBattle.charaList.ForEach(chara => chara.smallLock.SetActive(false));
        //启动目标模型的大框
        CurrentSelectTargets.ForEach(chara => chara.largeLock.SetActive(true));
        _ = ChangeLargeLockSize();
        //若果是扩散，启动两侧模型的小框
        if (currentActionData.SkillTags.Contains(SkillTag.Diffusion))
        {
