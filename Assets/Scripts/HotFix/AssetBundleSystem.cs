...
public class AssetBundleSystem
{
    ...
    /// <returns></returns>
    public static async Task Init(string ServerTag, bool isHotFixedLoad = true,List<string> filterFileName = null)
    {
        ...
        {
            targetPath = Application.persistentDataPath + $"/AssetBundles/{(Application.isMobilePlatform ? "Android" : ServerTag)}/";
        }
        ...
        {
            //存在过滤文件列表的话就只加载列表中的ab包
            if (filterFileName != null)
            {
                if (!filterFileName.Contains(file.Name))
                {
                    continue;
                }
            }
    ...
    public static T Load<T>(string tag, string fileName) where T : UnityEngine.Object
    {
        ...
        if (targetAssets != null)
        {
            ...
            if (targetAsset == null)
            {
                ...
                targetAsset = targetAssets.FirstOrDefault(asset => asset.name == "404" && asset.GetType() == typeof(T));
            }
