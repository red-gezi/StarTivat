...
public class GameEventManager
{
    ...
    {
        Debug.Log("进入房间");
        roomData.BelongBuffList = GameManager.CurrentBuffList;
        roomData.ListenerBuffs = OutBattleManager.GetCurrentBuff();
        roomData.exceBuff = GameManager.baseBuff;
        await TriggerTargetEventAsync(BuffEventType.EnterRoom, roomData);
    }
    public static async Task DestoryObject(RoomData roomData)
    {
        ...
        roomData.ListenerBuffs = OutBattleManager.GetCurrentBuff();
        roomData.exceBuff = GameManager.baseBuff;
        await TriggerTargetEventAsync(BuffEventType.DestoryObject, roomData);
    }
    //获得奇物,可指定id，-1代表随机
    ...
    public static async Task ElementalReaction(ElementalReactionData data)
    {
        ...
        await TriggerTargetEventAsync(BuffEventType.ElementalReaction, data);
        return;
    }
    //通用的广播角色某个事件
    public static async Task BroadcastCharaEvent(BuffEventType eventType, SkillData skillData)
    {
        ...
        await TriggerTargetEventAsync(eventType, skillData);
        return;
    }
