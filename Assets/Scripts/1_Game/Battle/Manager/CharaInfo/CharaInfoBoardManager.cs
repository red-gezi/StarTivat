...
public class CharaInfoBoardManager : MonoBehaviour
{
    ...
    public void SwitchCurrentChara(int index)
    {
        ...
        var character = isShowEnemy
            ? BattleManager.Instance.EnemyList[index]
            : BattleManager.Instance.PlayerList[index];
    ...
    public void RefreshCharaList()
    {
        int count = isShowEnemy ?
            ...
            BattleManager.Instance.PlayerList.Count;
        for (int i = 0; i < 5; i++)
        {
            ...
            {
                var character = isShowEnemy
                ? BattleManager.Instance.EnemyList[i]
                : BattleManager.Instance.PlayerList[i];
