...
class CharaBoardManager : MonoBehaviour
{
    ...
    public static void Init()
    {
        ...
        for (int i = charaBoardList.Count; i < BattleManager.Instance.PlayerList.Count; i++)
        {
        ...
        //刷新每个面板的数据
        for (int i = 0; i < BattleManager.Instance.PlayerList.Count; i++)
        {
            //开启需要数量的角色面板
            charaBoardList[i].SetActive(true);
            //设置角色图标
            charaBoardList[i].transform.GetChild(0).GetComponent<Image>().sprite = BattleManager.Instance.PlayerList[i].miniCharaIcon;
            //设置元素爆发图标
            charaBoardList[i].transform.GetChild(1).GetComponent<Image>().sprite = BattleManager.Instance.PlayerList[i].brustSkillIcon;
            //设置元素爆发图标边缘颜色
            ...
            charaBoardList[i].transform.GetChild(1).GetComponent<Image>().material.color = BattleManager.Instance.PlayerList[i].CurrentCharaData.PlayerElement switch
            {
    ...
    public static void Refresh()
    {
        ...
        for (int i = 0; i < BattleManager.Instance.PlayerList.Count; i++)
        {
            ...
            //刷新角色血量
            charaBoardList[i].transform.GetChild(2).GetChild(0).GetComponent<TextMeshProUGUI>().text = BattleManager.Instance.PlayerList[i].CurrentCharaData.CurrentHealthPoints.ToString();
            //刷新角色盾量
            //刷新角色状态
    ...
    public void ClickBrust(GameObject charaBoard)
    {
        ...
        var chara = BattleManager.Instance.PlayerList[index];
