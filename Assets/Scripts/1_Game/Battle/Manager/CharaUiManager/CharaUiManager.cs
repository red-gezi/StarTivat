...
public class CharaUiManager : MonoBehaviour
{
    ...
    public static async Task CreatNumber(bool isCritical, Character target, ElementType elementType, int point)
    {
        ...
        {
            pointPrefab.transform.localScale = Vector3.one * (1 + progress);
        });
        await CustomThread.TimerAsync(0.2f, progress =>
        {
    ...
    public static async Task CreatReactionText(Character target, ReactionType reaction)
    {
        ...
        reactionPrefab.transform.localPosition = screenPosition;
        reactionPrefab.transform.localScale = Vector3.one;
        await CustomThread.TimerAsync(0.2f, progress =>
        {
            reactionPrefab.transform.localScale = Vector3.one * (1 + progress);
        });
        await CustomThread.TimerAsync(0.2f, progress =>
        {
    ...
    {
        var targets = BattleManager.Instance.EnemyList.Select(enemy => enemy.gameObject).ToList();
        for (int i = 0; i < 10; i++)
        {
